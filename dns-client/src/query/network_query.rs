...
pub async fn query_network<CCache>(client: &DNSAsyncClient, cache: Arc<CCache>, question: &Question, name_server_address: &IpAddr) -> io::Result<Message> where CCache: AsyncCache + Sync {
    ...
    let message = MixedSocket::query(socket.clone(), message_question.clone(), QueryOptions::Both, Some(QUERY_TIMEOUT_MS), None).await?;
    ...
    let message = MixedSocket::query(socket, message_question, QueryOptions::TcpOnly, Some(QUERY_TIMEOUT_MS), None).await?;
    cache.insert_message(&message).await;
    return Ok(message);
}
