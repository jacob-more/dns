...
pub async fn query_network<CCache>(client: &DNSAsyncClient, cache: Arc<CCache>, question: &Question, name_server_address: &IpAddr) -> io::Result<Message> where CCache: AsyncCache + Sync {
    ...
    let message = MixedSocket::query(&socket, &mut message_question, QueryOptions::Both, Some(QUERY_TIMEOUT_MS)).await?;
    ...
    let message = MixedSocket::query(&socket, &mut message_question, QueryOptions::TcpOnly, Some(QUERY_TIMEOUT_MS)).await?;
