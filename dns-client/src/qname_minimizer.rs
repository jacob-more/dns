...
where
    I: 'a + Iterator<Item = CDomainName> + ExactSizeIterator + DoubleEndedIterator
{
    pub fn new_limited_minimizer(qname: &'a CDomainName, search_names: I, qname_minimization_limit: usize) -> Self {
    ...
    I: 'a + Iterator<Item = CDomainName> + ExactSizeIterator + DoubleEndedIterator
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            QNameMinimizer::None { qname: _, repeat_n: 0 } => {
                return None;
            },
            ...
            QNameMinimizer::LimitedMinimizer { qname, remaining_minimized_qnames: 0, qname_iter } => {
                ...
                if remaining_iterations == 0 {
                    *self = QNameMinimizer::None { qname, repeat_n: remaining_iterations };
                    return None;
                } else {
                    let returned_qname = qname.clone();
                    *self = QNameMinimizer::None { qname, repeat_n: remaining_iterations - 1 };
                    return Some(returned_qname);
                }
            },
            QNameMinimizer::LimitedMinimizer { qname: _, remaining_minimized_qnames, qname_iter } => {
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        match self {
            ...
            QNameMinimizer::LimitedMinimizer { qname: _, remaining_minimized_qnames: _, qname_iter } => qname_iter.size_hint(),
        }
    }
}
    ...
    I: 'a + Iterator<Item = CDomainName> + ExactSizeIterator + DoubleEndedIterator
{
    fn len(&self) -> usize {
        match self {
            ...
            QNameMinimizer::LimitedMinimizer { qname: _, remaining_minimized_qnames: _, qname_iter } => qname_iter.len(),
        }
    }
}
