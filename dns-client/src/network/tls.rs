...
use dns_lib::{
    ...
    types::domain_name::{CompressionMap, DomainVec},
};
...
use crate::network::{
    ...
    socket::{
        ...
        tls::{QTlsSocket, QTlsSocketProj, TlsReadHalf, TlsState},
    },
};
                ...
                InnerTQProj::Running {
                    ...
                    match (send_query.as_mut().project(), tq_socket.as_mut().project()) {
                        ...
                        (
                            ...
                            println!(
                                "Sending on TLS socket {} {{ drop rate {:.2}%, response time {:.2} ms, timeout {} ms }} :: {:?}",
                                this.socket.upstream_address,
                                this.socket.average_dropped_tls_packets() * 100.0,
                                this.socket.average_tls_response_time(),
                                this.tls_timeout.as_millis(),
                                this.query
                            );
                            ...
                            let send_query_future = async move {
                                ...
                                let bytes_written =
                                    match w_tls_stream.write(&raw_message[..wire_length]).await {
                                        Ok(bytes) => bytes,
                                        Err(error) => {
                        ...
                        (QSendProj::Complete(_), _) => {
                            ...
                            match this.result_receiver.as_mut().poll(cx) {
                                Poll::Ready(Ok(Ok(_))) => {
                                    ...
                                    this.inner
                                        .set_cleanup(TlsResponseTime::Responded(execution_time));
                ...
                InnerTQProj::Cleanup(execution_time) => {
                    ...
                    match execution_time {
                        TlsResponseTime::Dropped => {
                            let average_tls_dropped_packets =
                                this.socket.add_dropped_packet_to_tls_average();
                        ...
                        TlsResponseTime::Responded(response_time) => {
                            let (average_tls_response_time, average_tls_dropped_packets) =
                                this.socket.add_response_time_to_tls_average(*response_time);
                            if average_tls_dropped_packets.current_average()
                                ...
                                w_active_queries.tls_timeout = bound(
                                    max(
                                        ...
                                        Duration::from_secs_f64(
                                            average_tls_response_time.current_average()
...
impl TlsSocket {
    ...
    async fn listen_tls_cleanup(self: Arc<Self>, kill_tls: AwakeToken) {
        ...
        match &*w_state {
            ...
            TlsState::Establishing { sender: _, kill: _ } | TlsState::None | TlsState::Blocked => {
                // This is not our socket to clean up.
                drop(w_state);
            }
        }
...
impl TlsSocket {
    ...
    #[inline]
    fn add_response_time_to_tls_average(
        &self,
