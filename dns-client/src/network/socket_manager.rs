...
use dns_lib::types::domain_name::DomainVec;
...
struct InternalSocketManager {
    ...
    tls_sockets: HashMap<(IpAddr, DomainVec), (Arc<TlsSocket>, u8)>,
    quic_sockets: HashMap<(IpAddr, DomainVec), (Arc<QuicSocket>, u8)>,
...
impl SocketManager {
    ...
    /// This function is cancel safe.
    #[inline]
    pub async fn get_tls(&self, address: (IpAddr, DomainVec)) -> Arc<TlsSocket> {
        let r_socket_manager = self.internal.read().await;
    ...
    /// This function is cancel safe.
    #[inline]
    pub async fn try_get_tls(&self, address: &(IpAddr, DomainVec)) -> Option<Arc<TlsSocket>> {
        let r_socket_manager = self.internal.read().await;
    ...
    pub async fn get_all_tls(
        &self,
        addresses: impl Iterator<Item = (IpAddr, DomainVec)>,
    ) -> Vec<Arc<TlsSocket>> {
        let mut w_socket_manager = self.internal.write().await;
        let sockets = addresses
            .map(
    ...
    where
        Self: Sized,
        F: FnMut((&(IpAddr, DomainVec), &Arc<TlsSocket>)),
    {
        let r_socket_manager = self.internal.read().await;
        r_socket_manager
            .tls_sockets
    ...
    /// This function is cancel safe.
    #[inline]
    pub async fn get_quic(&self, address: (IpAddr, DomainVec)) -> Arc<QuicSocket> {
        let r_socket_manager = self.internal.read().await;
    ...
    /// This function is cancel safe.
    #[inline]
    pub async fn try_get_quic(&self, address: &(IpAddr, DomainVec)) -> Option<Arc<QuicSocket>> {
        let r_socket_manager = self.internal.read().await;
    ...
    where
        Self: Sized,
        F: FnMut((&(IpAddr, DomainVec), &Arc<QuicSocket>)),
    {
        let r_socket_manager = self.internal.read().await;
        r_socket_manager
            .quic_sockets
