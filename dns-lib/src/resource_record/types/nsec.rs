    ...
    fn from_tokenized_rdata(rdata: &Vec<&str>) -> Result<Self, crate::serde::presentation::errors::TokenizedRecordError> where Self: Sized {
        match rdata.as_slice() {
            ...
            &[next_domain_name, ..] => {
                ...
                Ok(Self { next_domain_name, type_bit_map })
            },
        }
    }
}
...
mod circular_serde_sanity_test {
    use crate::{resource_record::rtype::RType, serde::wire::circular_test::gen_test_circular_serde_sanity_test, types::{domain_name::DomainName, rtype_bitmap::RTypeBitmap}};
    ...
    gen_test_circular_serde_sanity_test!(
        ...
        NSEC {
            ...
            type_bit_map: RTypeBitmap::from_rtypes([RType::A, RType::MX, RType::RRSIG, RType::NSEC, RType::Unknown(1234)].iter())
        }
    );
}
