...
pub struct MG {
    mg_domain_name: CompressibleDomainVec,
}

impl MG {
    #[inline]
    pub fn new(mg_domain_name: DomainNameVec) -> Self {
        Self {
            mg_domain_name: CompressibleDomainVec(mg_domain_name),
        }
    }

    #[inline]
    pub fn mailbox_group_domain_name(&self) -> &DomainNameVec {
        &self.mg_domain_name
    }
}
...
mod tokenizer_tests {
    ...
    gen_ok_record_test!(
        ...
        MG,
        MG {
            mg_domain_name: CompressibleDomainVec(DomainNameVec::from_utf8(GOOD_DOMAIN).unwrap())
        },
        [GOOD_DOMAIN]
    );
