...
use crate::serde::{
    presentation::{
        ...
        parse_chars::non_escaped_to_escaped::NonEscapedIntoEscapedIter,
...
impl AsciiString {
    ...
    pub fn from_range(&self, start: usize, end: usize) -> Self {
        Self {
            string: self.string[start..end].into(),
        }
    }
...
mod upper_lower_case_tests {
    ...
    const MIXED_CASE_STRING: &str =
        "This Is A Test aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ 0123456789 !@#$%^&*()";
    const UPPER_STRING: &str =
        "THIS IS A TEST AABBCCDDEEFFGGHHIIJJKKLLMMNNOOPPQQRRSSTTUUVVWWXXYYZZ 0123456789 !@#$%^&*()";
    const LOWER_STRING: &str =
        "this is a test aabbccddeeffgghhiijjkkllmmnnooppqqrrssttuuvvwwxxyyzz 0123456789 !@#$%^&*()";

    #[test]
    fn test_as_upper_success() {
        // Setup
        let ascii_mixed_case = AsciiString::from_utf8(MIXED_CASE_STRING)
            .expect("The mixed case string could not convert from utf8 to ascii");
        let ascii_upper_case = AsciiString::from_utf8(UPPER_STRING)
            .expect("The upper case string could not convert from utf8 to ascii");
        let ascii_lower_case = AsciiString::from_utf8(LOWER_STRING)
            .expect("The lower case string could not convert from utf8 to ascii");

        let expected_string = AsciiString::from_utf8(UPPER_STRING)
            .expect("The upper case string could not convert from utf8 to ascii");
    ...
    fn test_upper_success() {
        // Setup
        let mut ascii_mixed_case = AsciiString::from_utf8(MIXED_CASE_STRING)
            .expect("The mixed case string could not convert from utf8 to ascii");
        let mut ascii_upper_case = AsciiString::from_utf8(UPPER_STRING)
            .expect("The upper case string could not convert from utf8 to ascii");
        let mut ascii_lower_case = AsciiString::from_utf8(LOWER_STRING)
            .expect("The lower case string could not convert from utf8 to ascii");
    ...
    fn test_as_lower_success() {
        ...
        let expected_string = AsciiString::from_utf8(LOWER_STRING)
            .expect("The upper case string could not convert from utf8 to ascii");
    ...
    const MIXED_CASE_ALPHA_NUMERIC: &str =
        "aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ0123456789";
    const UPPER_STRING_ALPHA_NUMERIC: &str =
        "AABBCCDDEEFFGGHHIIJJKKLLMMNNOOPPQQRRSSTTUUVVWWXXYYZZ0123456789";
    const LOWER_STRING_ALPHA_NUMERIC: &str =
        "aabbccddeeffgghhiijjkkllmmnnooppqqrrssttuuvvwwxxyyzz0123456789";

    const MIXED_CASE_NON_ALPHA_NUMERIC: &str =
        "aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ0123456789!@#$%^&*()";
    const UPPER_STRING_NON_ALPHA_NUMERIC: &str =
        "AABBCCDDEEFFGGHHIIJJKKLLMMNNOOPPQQRRSSTTUUVVWWXXYYZZ0123456789!@#$%^&*()";
    const LOWER_STRING_NON_ALPHA_NUMERIC: &str =
        "aabbccddeeffgghhiijjkkllmmnnooppqqrrssttuuvvwwxxyyzz0123456789!@#$%^&*()";

    #[test]
    fn test_is_alpha_numeric_lower() {
        // Setup
        let ascii_mixed_case_alpha = AsciiString::from_utf8(MIXED_CASE_ALPHA)
            .expect("The mixed case string could not convert from utf8 to ascii");
        let ascii_upper_case_alpha = AsciiString::from_utf8(UPPER_STRING_ALPHA)
            .expect("The upper case string could not convert from utf8 to ascii");
        let ascii_lower_case_alpha = AsciiString::from_utf8(LOWER_STRING_ALPHA)
            .expect("The lower case string could not convert from utf8 to ascii");

        let ascii_numeric = AsciiString::from_utf8(NUMERIC)
            .expect("The numeric string could not convert from utf8 to ascii");

        let ascii_mixed_case_alphanumeric = AsciiString::from_utf8(MIXED_CASE_ALPHA_NUMERIC)
            .expect("The mixed case string could not convert from utf8 to ascii");
        let ascii_upper_case_alphanumeric = AsciiString::from_utf8(UPPER_STRING_ALPHA_NUMERIC)
            .expect("The upper case string could not convert from utf8 to ascii");
        let ascii_lower_case_alphanumeric = AsciiString::from_utf8(LOWER_STRING_ALPHA_NUMERIC)
            .expect("The lower case string could not convert from utf8 to ascii");

        let ascii_mixed_case_non_alphanumeric =
            AsciiString::from_utf8(MIXED_CASE_NON_ALPHA_NUMERIC)
                .expect("The mixed case string could not convert from utf8 to ascii");
        let ascii_upper_case_non_alphanumeric =
            AsciiString::from_utf8(UPPER_STRING_NON_ALPHA_NUMERIC)
                .expect("The upper case string could not convert from utf8 to ascii");
        let ascii_lower_case_non_alphanumeric =
            AsciiString::from_utf8(LOWER_STRING_NON_ALPHA_NUMERIC)
                .expect("The lower case string could not convert from utf8 to ascii");
