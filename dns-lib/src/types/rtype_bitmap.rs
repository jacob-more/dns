...
use crate::{
    ...
    serde::{
        presentation::{from_presentation::FromPresentation, to_presentation::ToPresentation},
...
impl WindowBlock {
    ...
    #[inline]
    pub fn to_rtypes<'a, 'b>(&'a self) -> impl Iterator<Item = RType> + 'b
    where
        'a: 'b,
    {
        ...
        // bit is in the set.
        static MASK_AND_OFFSET: [(u8, u16); 8] = [
            (0b1000_0000, 0),
            (0b0100_0000, 1),
            (0b0010_0000, 2),
            (0b0001_0000, 3),
        ...
        self.map
            .iter()
            .enumerate()
            .flat_map(move |(bye_index, byte)| {
                MASK_AND_OFFSET.iter().filter_map(move |(mask, offset)| {
                    if (byte & mask) == *mask {
                        Some(RType::from_code(
                            ((block_number * 256) + (bye_index as u16 * 8)) + offset,
                        ))
                    } else {
                        None
                    }
...
impl FromWire for RTypeBitmap {
    ...
    {
        Ok(RTypeBitmap {
            blocks: <Vec<WindowBlock>>::from_wire_format(wire)?,
        })
    }
}
...
impl FromPresentation for RTypeBitmap {
    fn from_token_format<'a, 'b, 'c, 'd>(
        tokens: &'c [&'a str],
    ) -> Result<(Self, &'d [&'a str]), crate::serde::presentation::errors::TokenError>
    where
        Self: Sized,
        'a: 'b,
        'c: 'd,
...
mod circular_sanity_tests {
    ...
    use crate::{
        resource_record::rtype::RType,
        serde::wire::{
            circular_test::gen_test_circular_serde_sanity_test, to_wire::ToWire,
    ...
    circular_sanity_test!(
        test_1_rtypes_from_to_collection,
        test_1_rtypes_wire,
        vec![RType::A]
    );
    circular_sanity_test!(
        test_2_rtypes_from_to_collection,
        test_2_rtypes_wire,
        vec![RType::A, RType::AAAA]
    );
    circular_sanity_test!(
        test_3_rtypes_from_to_collection,
        test_3_rtypes_wire,
        vec![RType::A, RType::AAAA, RType::NS]
    );
    circular_sanity_test!(
        test_4_rtypes_from_to_collection,
        test_4_rtypes_wire,
        vec![RType::A, RType::AAAA, RType::NS, RType::HTTPS]
    );
    circular_sanity_test!(
        test_5_rtypes_from_to_collection,
        test_5_rtypes_wire,
        vec![RType::A, RType::NS, RType::MF, RType::CNAME, RType::SOA]
    );
    circular_sanity_test!(
        test_6_rtypes_from_to_collection,
        test_6_rtypes_wire,
        vec![
            RType::TKEY,
            RType::TSIG,
    ...
    circular_sanity_test!(
        test_7_rtypes_from_to_collection,
        test_7_rtypes_wire,
        vec![
            RType::HIP,
            RType::NINFO,
    ...
    circular_sanity_test!(
        test_8_rtypes_from_to_collection,
        test_8_rtypes_wire,
        vec![
            RType::NS,
            RType::NULL,
    ...
    circular_sanity_test!(
        test_9_rtypes_from_to_collection,
        test_9_rtypes_wire,
        vec![
            RType::NS,
            RType::NULL,
    ...
    circular_sanity_test!(
        test_10_rtypes_from_to_collection,
        test_10_rtypes_wire,
        vec![
            RType::NS,
            RType::NULL,
    ...
    circular_sanity_test!(
        test_11_rtypes_from_to_collection,
        test_11_rtypes_wire,
        vec![
            RType::A,
            RType::NS,
    ...
    circular_sanity_test!(
        test_12_rtypes_from_to_collection,
        test_12_rtypes_wire,
        vec![
            RType::A,
            RType::NS,
    ...
    circular_sanity_test!(
        test_12_rtypes_2_repeats_from_to_collection,
        test_12_rtypes_2_repeats_wire,
        vec![
            RType::SOA,
            RType::NS,
            RType::MF,
    ...
    fn rfc3845_example_test() {
        ...
        let expected_wire_format: Vec<u8> = vec![
            0x00, 0x06, 0x40, 0x01, 0x00, 0x00, 0x00, 0x03, 0x04, 0x1b, 0x00, 0x00, 0x00, 0x00,
