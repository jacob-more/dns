...
pub trait ToWire {
    fn to_wire_format<'a, 'b>(
        &self,
        wire: &'b mut WriteWire<'a>,
        compression: &mut Option<CompressionMap>,
    ) -> Result<(), WriteWireError>
    where
        'a: 'b;
    ...
    ($int_name:literal, $int_type:ty, $byte_count:ident) => {
        impl ToWire for $int_type {
            ...
            fn to_wire_format<'a, 'b>(
                &self,
                wire: &'b mut WriteWire<'a>,
                _compression: &mut Option<CompressionMap>,
            ) -> Result<(), WriteWireError>
            where
                'a: 'b,
            {
...
macro_rules! ux_to_wire_impl {
    ($int_name:literal, $int_type:ty, $byte_count:ident, $super_type:ty, $super_byte_count:ident) => {
        impl ToWire for $int_type {
            ...
            {
                wire.write_bytes(
                    &(<$super_type>::from(*self)).to_be_bytes()
                        [(($super_byte_count as usize) - ($byte_count as usize))..],
                )
            }
...
impl<T: ToWire> ToWire for Vec<T> {
    ...
    #[inline]
    fn serial_length(&self) -> u16 {
        self.iter().map(|x| x.serial_length()).sum()
    }
}
