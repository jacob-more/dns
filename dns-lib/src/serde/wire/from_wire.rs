...
use ux::{
    i24, i40, i48, i56, i72, i80, i88, i96, i104, i112, i120, u1, u3, u4, u7, u24, u40, u48, u56,
    u72, u80, u88, u96, u104, u112, u120,
};
...
pub trait FromWire {
    fn from_wire_format<'a, 'b>(wire: &'b mut ReadWire<'a>) -> Result<Self, ReadWireError>
    where
        Self: Sized,
        'a: 'b;
}
    ...
    ($int_name:literal, $int_type:ty, $byte_count:ident) => {
        impl FromWire for $int_type {
                ...
                'a: 'b,
            {
                ...
                let bytes = wire
                    .take_or_err($byte_count as usize, || {
                        format!(
                            "could not read {}; wire length less than {} bytes",
                            $int_name, $byte_count
                        )
                    })?
                    .try_into()
    ...
    ($int_name:literal, $int_type:ty, $int_byte_count:ident, $super_type:ty, $super_byte_count:ident) => {
        impl FromWire for $int_type {
            ...
            where
                ...
                'a: 'b,
            {
                let bytes = wire.take_or_err($int_byte_count as usize, || {
                    format!(
                        "could not read {}; wire length less than {} bytes",
                        $int_name, $int_byte_count
                    )
                })?;
                let bytes: Vec<u8> = [0_u8]
                    .iter()
                    .cycle()
                    .take(($super_byte_count as usize) - ($int_byte_count as usize))
                    .chain(bytes.iter())
                    .map(|x| *x)
                    .collect();
                // `unwrap()` is safe in this case because it will always have a fixed size.
...
impl<T: FromWire + Default, const SIZE: usize> FromWire for TinyVec<[T; SIZE]>
where
    [T; SIZE]: tinyvec::Array<Item = T>,
{
    /// Consumes the entire wire buffer.
    #[inline]
    fn from_wire_format<'a, 'b>(wire: &'b mut ReadWire<'a>) -> Result<Self, ReadWireError>
...
impl<T: FromWire + Default, const SIZE: usize> FromWire for ArrayVec<[T; SIZE]>
where
    [T; SIZE]: tinyvec::Array<Item = T>,
{
    /// Consumes the entire wire buffer.
...
impl FromWire for Ipv4Addr {
    ...
    {
        // `unwrap()` is safe in this case because it will always have a fixed size.
        let bytes: [u8; IPV4_BYTE_COUNT as usize] = wire
            .take_or_err(IPV4_BYTE_COUNT as usize, || {
                format!("IPv4 Addresses must be {IPV4_BYTE_COUNT} bytes in length")
            })?
            .try_into()
            .unwrap();
...
impl FromWire for Ipv6Addr {
    ...
    {
        // `unwrap()` is safe in this case because it will always have a fixed size.
        let bytes: [u8; IPV6_BYTE_COUNT as usize] = wire
            .take_or_err(IPV6_BYTE_COUNT as usize, || {
                format!("IPv6 Addresses must be {IPV6_BYTE_COUNT} bytes in length")
            })?
            .try_into()
            .unwrap();
...
impl FromWire for MacAddress {
    ...
    {
        // `unwrap()` is safe in this case because it will always have a fixed size.
        let bytes: [u8; MAC_ADDRESS_BYTE_COUNT as usize] = wire
            .take_or_err(MAC_ADDRESS_BYTE_COUNT as usize, || {
                format!("Mac Addresses must be {MAC_ADDRESS_BYTE_COUNT} bytes in length")
            })?
            .try_into()
            .unwrap();
...
impl FromWire for (u1, u7) {
        ...
        'a: 'b,
    {
        ...
        let bit_6to0 = u7::new(bit_7to0 & 0b01111111);
...
impl FromWire for (u1, u3, u4) {
        ...
        'a: 'b,
    {
        ...
        let bit_3to0 = u4::new(input & 0b00001111);
...
impl FromWire for (u1, u4, u1, u1, u1) {
        ...
        'a: 'b,
    {
        ...
        let bit_0 = u1::new(input & 0b00000001);
