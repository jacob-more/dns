...
use dns_lib::{
    interface::{
        cache::{MetaAuth, main_cache::AsyncMainCache},
        client::{AsyncClient, Context, QNameMinimization},
    },
...
async fn main() {
    ...
    rustls::crypto::ring::default_provider()
        .install_default()
        .expect("Failed to set Ring as the default crypto provider");
    ...
    cache
        .load_from_file(&mut file, MetaAuth::NotAuthoritativeBootstrap)
        .await
        .unwrap();
    ...
    if !std::io::stdin().is_terminal() {
        search_domains.extend(std::io::stdin().lines().filter_map(|maybe| match maybe {
...
async fn test_many_dns(client: Arc<DNSAsyncClient>, domains: impl Iterator<Item = DomainNameVec>) {
    ...
    let domain_rtype_pairs = domains
        .map(|domain| {
            SEARCH_RTYPES
                .into_iter()
                .map(move |rtype| (domain.clone(), rtype))
        })
        .flatten();
    futures::stream::iter(domain_rtype_pairs)
        .for_each_concurrent(None, |(domain, rtype)| {
...
async fn query_dn(client: Arc<DNSAsyncClient>, rtype: RType, domain: DomainNameVec) {
    ...
    let response = DNSAsyncClient::query(
        client,
        Context::new(
            question.clone(),
            QNameMinimization::All {
                primary_minimization_limit: 10,
                ns_minimization_limit: 8,
                sub_ns_minimization_limit: 6,
            },
        ),
    )
    .await;
